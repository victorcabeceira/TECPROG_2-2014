...
int main()
{
	...
	printf ("\n** UTF-8 **\n");
	{
		...
		else
		{
			...
			if ( saved && verify )
			{
				...
				{
					// Added ignoreWarning1 to remove warning.
					char* ignoreWarning1 = fgets( savedBuf, 256, saved );
					((void) ignoreWarning1);
	...
	// Commented to remove unused warnings.
	/*{
...

	{
		...
		{
			char buf[ 1024 ];
			// Added ignoreWarning2 to remove warning.
			char* ignoreWarning2 = fgets( buf, 1024, textfile );
			((void) ignoreWarning2);
